...
<%
    ...
    String countSql = "SELECT COUNT(*) AS cnt " +
            ...
            "WHERE (r.inventory_id, r.rental_date) IN (" +
    ...
    String sql = "SELECT r.inventory_id, r.rental_date, r.return_date, f.title AS film_title " +
            "FROM rental r " +
            "LEFT JOIN inventory i ON r.inventory_id = i.inventory_id " +
            "INNER JOIN film f ON i.film_id = f.film_id " +
            "WHERE (r.inventory_id, r.rental_date) IN (" +
    ...
    while (rs.next()) {
    	HashMap<String,Object> rental = new HashMap<>();
    		...
    		rental.put("return_date", rs.getString("return_date"));
    		rental.put("film_title", rs.getString("film_title"));
	...
	<h1>영화 비디오 대여 리스트</h1>
    	...
    	%>
            <tr>
                ...
                <td><%= rental.get("rental_date") %></td>
                <td><%= rental.get("return_date") %></td>
                <td><%= rental.get("film_title") %></td>
                <td><input type="submit" value="뱐납"> </td>
            </tr>
    	<%
        	}
   			 %>
